...
// 2.- reset pulse for LoRa module initialization AGGE
  ...
  // state = setLORA();
	...
	/*
	writeRegister(0x0,0x0);     /// RegFifo
	writeRegister(0x1,0x81);    /// RegOpMode
	writeRegister(0x2,0x1A);    /// unused in lora Why ?
	writeRegister(0x3,0xB);     /// unused in lora Why ?
	writeRegister(0x4,0x0);     /// unused in lora Why ?
	writeRegister(0x5,0x52);    /// unused in lora Why ?
	writeRegister(0x6,0xD8);    /// RegFrfMsb
	writeRegister(0x7,0x99);    /// RegFrfMid
	writeRegister(0x8,0x99);    /// RegFrfLsb
	writeRegister(0x9,0x0);     /// RegPaConfig
	writeRegister(0xA,0x9);     /// RegPaRamp
	writeRegister(0xB,0x3B);    /// RegOcp
	writeRegister(0xC,0x23);    /// RegLna
	writeRegister(0xD,0x1);     /// RegFifoAddrPtr
	writeRegister(0xE,0x80);    /// RegFifoTxBaseAddr
	writeRegister(0xF,0x0);     /// RegFifoRxBaseAddr
	writeRegister(0x10,0x0);    /// FifoRxCurrentAddr
	writeRegister(0x11,0x0);    /// RegIrqFlagsMask
	writeRegister(0x12,0x0);    /// RegIrqFlags
	writeRegister(0x13,0x0);    /// RegRxNbBytes
	writeRegister(0x14,0x0);    /// RegRxHeaderCntValueMsb
	writeRegister(0x15,0x0);    /// RegRxHeaderCntValueLsb
	writeRegister(0x16,0x0);    /// RegRxPacketCntValueMsb
	writeRegister(0x17,0x0);    /// RegRxPacketCntValueLsb
	writeRegister(0x18,0x10);   /// RegModemStat
	writeRegister(0x19,0x0);    /// RegPktSnrValue
	writeRegister(0x1A,0x0);    /// RegPktRssiValue
	writeRegister(0x1B,0x0);    /// RegRssiValue
	writeRegister(0x1C,0x0);    /// RegHopChannel
	writeRegister(0x1D,0x4A);   /// RegModemConfig1
	writeRegister(0x1E,0x97);   /// RegModemConfig2
	writeRegister(0x1F,0xFF);   /// RegSymbTimeoutLsb
	writeRegister(0x20,0x0);    /// RegPreambleMsb
	writeRegister(0x21,0x8);    /// RegPreambleLsb
	writeRegister(0x22,0xFF);   /// RegPayloadLength
	writeRegister(0x23,0xFF);   /// RegMaxPayloadLength
	writeRegister(0x24,0x0);    /// RegHopPeriod
	writeRegister(0x25,0x0);    /// RegFifoRxByteAddr
	writeRegister(0x26,0x0);    /// RegModemConfig3
	writeRegister(0x27,0x0);    /// RegPpmCorrection
	writeRegister(0x28,0x0);    /// RegFeiMsb
	writeRegister(0x29,0x0);    /// RegFeiMid
	writeRegister(0x2A,0x0);    /// RegFeiLsb
	writeRegister(0x2B,0x0);    /// RESERVED
	writeRegister(0x2C,0x0);    /// RegRssiWideband
	writeRegister(0x2D,0x50);   /// RESERVED Why ?
	writeRegister(0x2E,0x14);   /// RESERVED Why ?
	writeRegister(0x2F,0x40);   /// RESERVED Why ?
	writeRegister(0x30,0x0);    /// RESERVED Why ?
	writeRegister(0x31,0x3);    /// RegDetectOptimize
	writeRegister(0x32,0x5);    /// RESERVED Why ?
	writeRegister(0x33,0x27);   /// RegInvertIQ
	writeRegister(0x34,0x1C);   /// RESERVED Why ?
	writeRegister(0x35,0xA);    /// RESERVED Why ?
	writeRegister(0x36,0x0);    /// RESERVED Why ?
	writeRegister(0x37,0xA);    /// RegDetectionThreshold
	writeRegister(0x38,0x42);   /// RESERVED Why ?
	writeRegister(0x39,0x12);   /// RegSyncWord
	writeRegister(0x3A,0x65);   /// RESERVED Why ?
	writeRegister(0x3B,0x1D);   /// RESERVED Why ?
	writeRegister(0x3C,0x1);    /// RESERVED Why ?
	writeRegister(0x3D,0xA1);   /// RESERVED Why ?
	writeRegister(0x3E,0x0);    /// RESERVED Why ?
	writeRegister(0x3F,0x0);    /// RESERVED Why ?
	writeRegister(0x40,0x0);    /// RegDioMapping1
	writeRegister(0x41,0x0);    /// RegDioMapping2
	writeRegister(0x42,0x22);   /// RegVersion
    */
...
byte SX1272::readRegister(byte address)
{
    byte value = 0x00;
		...
		if (address < 16 )Serial.print("0x0");
